...
errormessage = '\033[1m'+"\nMake sure code in login.txt is correct and you are on same pc. If the problem will continue to occur, please take your dwo finger and put it in your ass"+'\033[0m'
...
def Panding():
    ...
    try:
        ftxt = open("login.txt", "r")
        datatxt = ftxt.readline()
        ftxt.close()
        #json loader
        fjson = requests.get("http://xxx.kvalitne.cz/1.json")
        datajson = fjson.json()
        for i in datajson[datatxt]:
            pass
        fjson.close
        if HOST == i["ho1"]:
            #print("succesfuly done1")
            H = True
        else:
            #print("nope1")
            H = False
